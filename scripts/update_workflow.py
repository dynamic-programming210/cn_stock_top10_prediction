    ...
    steps:
      ...
      - name: Install dependencies
        run: |
          ...
          pip install snownlp  # Chinese NLP for news sentiment
      ...
      - name: Fetch news sentiment
        ...
        continue-on-error: true  # Don't fail if news fetching fails
        run: |
          echo "📰 Fetching news sentiment data..."
          python data/fetch_news.py
          echo "✅ News sentiment data updated!"
      
      - name: Build features
        run: |
          echo "🔧 Building features (including news sentiment)..."
      ...
      - name: Show latest predictions
        run: |
          echo "📋 Latest Top-10 Predictions:"
      ...
      - name: Commit and push updates
        run: |
          ...
          if git diff --staged --quiet; then
            ...
            git commit -m "🤖 Auto-update: predictions for $(date -u +%Y-%m-%d)"
            git push
